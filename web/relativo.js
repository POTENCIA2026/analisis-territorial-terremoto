...
(function(root){
  ...
  function render(state){
    ...
    const sector=result.definitions.find(s=>s.id===sectorId),year=String(state.date).slice(0,4);
    ...
    let html='<table>'+headers([percapita?'Municipio y prioridad per cápita':'Municipio y prioridad relativa',...result.definitions.map(s=>s.name)])+'<tbody>';
    ...
    $('relative-matrix').querySelectorAll('thead th').forEach((th,i)=>{
      if(!i)return;const s=result.definitions[i-1],active=s.id===sectorId;
